...
int main ()
{
	...
	while (!IS_BUTTON_PRESSED(platform.m_keyboard.key[KB_ESC]) && !platform.m_quit)
	{
		...
		TransformPush(&transform);
			...
			TransformPush(&transform);
				TransformTranslate(&transform, -0.4, 0, 0);
				Mtx44Identity(&rot);
				Mtx44Rotate(&rot, radians[0], 0, 0, 1);
				TransformMultiply(&transform, &rot);
				SetMtx44ByLocation(uMtx, TransformGetCurrentMatrix(&transform)->data);
				Draw(DRAW_TRIANGLE_STRIP, ELEMENT_BYTE, sizeof(indices)/sizeof(unsigned char), 0);

				TransformPush(&transform);
					TransformTranslate(&transform, 0, 0.3f, 0);
					SetMtx44ByLocation(uMtx, TransformGetCurrentMatrix(&transform)->data);
					Draw(DRAW_TRIANGLE_STRIP, ELEMENT_BYTE, sizeof(indices)/sizeof(unsigned char), 0);
			...
			TransformPush(&transform);
				Mtx44Identity(&rot);
				Mtx44Rotate(&rot, radians[1], 0, 0, 1);
				TransformMultiply(&transform, &rot);
				TransformTranslate(&transform, 0, -0.3f, 0);
				TransformMultiply(&transform, &rot);
				SetMtx44ByLocation(uMtx, TransformGetCurrentMatrix(&transform)->data);
